...
'''
bat '''
"%POWERSHELL%" -NoProfile -ExecutionPolicy Bypass -Command "Start-Sleep -Seconds 5; $health = Invoke-RestMethod [http://localhost:$env:HOST_PORT/health](http://localhost:$env:HOST_PORT/health); Write-Host 'Health check:'; $health | ConvertTo-Json; if ($health.status -ne 'UP') { throw 'Application health check failed' }; if ($health.version -ne $env:VERSION) { throw 'Version mismatch' }; if ($health.environment -ne $env:APP_ENV) { throw 'Environment mismatch' }"
'''
bat '''
"%POWERSHELL%" -NoProfile -ExecutionPolicy Bypass -Command "$db = Invoke-RestMethod [http://localhost:$env:HOST_PORT/db-health](http://localhost:$env:HOST_PORT/db-health); Write-Host 'Database health:'; $db | ConvertTo-Json; if ($db.database -ne 'UP') { throw 'Database connectivity check failed' }"
'''
bat '''
